...
export default class MockServer {
  ...
  items = [
    {
      type: 'folder',
      config: {id: '0', name: 'Documents', itemsAmount: '2'},
    },
    {
      type: 'folder',
      config: {id: '1', name: 'Images', itemsAmount: '2'},
    },
    {
      type: 'folder',
      config: {id: '2', name: 'Videos', itemsAmount: '1'},
    },
    {
      type: 'file',
      config: {id: '3', name: 'test.txt', mimeType: 'text', size: '20KB'},
    },
  ];
  ...
  constructor() {
    ...
    fetchMock
      .post('express:/rename-item/:id', ((url, request) => {
        const id = url.split('/')[2];
        this.items.forEach((item) => {
          if (item.config.id === id) {
            item.config.name = request.body.newItemName;
          }
        });
        return 200;
      }));
